...
`infra/bootstrap/` provisions the versioned remote-state bucket, and `infra/central/` decodes the deployment input and provisions the config bucket, source-state and delivery tables, FIFO queue and DLQ, Slack credential containers, operational topic, the chapter 05 IAM roles, and the log groups, dashboard, and alarms. It now also defines the Slack worker Lambda and FIFO event-source mapping when an exact package digest and S3 version are supplied; the other three handlers and their schedules remain milestones. `src/` holds the worker core, its Slack and credential adapters, and the AWS composition root. The FIFO handler preserves the ordered suffix after the first failure and emits bounded embedded metrics. [ADR-020](../adr/020-exact-application-version-gate-for-delivery.md) is implemented by a deterministic package builder, an append-only digest-key publisher, an exact S3 object-version input, and one `sha256:` environment value. Packages accumulate until a retirement tool can prove both retention floors. [ADR-007](../adr/007-central-slack-delivery-queue-and-worker.md) supplies the 300-second timeout boundary, six-times visibility, partial batch response, and destination-group ordering contract. Release publication has an S3 adapter already, because [ADR-019](../adr/019-s3-preconditions-for-release-publication-and-promotion.md) makes the store's error codes part of the contract rather than a deployment detail. Keep a concept in one owning document and link to it elsewhere. Do not copy full requirements between the goal, specification, ADRs, and runbook.
