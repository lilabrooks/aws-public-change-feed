...
`infra/bootstrap/` provisions the versioned remote-state bucket, and `infra/central/` decodes the deployment input and provisions the config bucket, source-state and delivery tables, FIFO queue and DLQ, Slack credential containers, operational topic, the chapter 05 IAM roles, and the log groups, dashboard, and alarms; the four Lambda functions and their schedules arrive with the runtime packages they host. `src/` currently holds normalization, matching, corpus evaluation, feed acquisition, announcement identity, feed-state ports, profile and route mapping, candidate construction, durable outbox creation, a DynamoDB delivery store, due-work dispatch, an SQS FIFO sender, release publication, promotion, and loading, the cross-document candidate rules, and the Slack delivery worker core behind its transport and credential ports. The candidate rules live in `src/` rather than in the validator because both the validator and the delivery worker apply them: a stored candidate is re-checked against the release it embeds before it is rendered, so a record edited after emission cannot post content the release never specified. [ADR-020](../adr/020-exact-application-version-gate-for-delivery.md) requires an exact application-version check before those rules run, preserving mismatched work for rollback rather than judging it with current semantics; its accepted replay revision keeps content-addressed packages for at least 400 days and at least 10 versions. The Slack HTTP transport and the credential readers for both accepted secret stores now sit behind the worker's transport and credential ports; the transport shares feed acquisition's address validation and pinned TLS connection through `pinned_https` rather than repeating them, which is what makes chapter 04's "the same anti-rebinding controls" one implementation instead of two. A received Slack status decides the outcome, so a body is read only for the one status that does not decide by itself; credential failures split into permanent and transient, and only the permanent half resolves delivery terminally. [ADR-007](../adr/007-central-slack-delivery-queue-and-worker.md) records both and uses the Lambda function timeout as the FIFO worker's capacity bound, with queue visibility at least six times that value. The Lambda handlers, the FIFO batch handler, the metrics adapter, recovery, and the remaining AWS adapters join them as those milestones land. Release publication has an S3 adapter already, because [ADR-019](../adr/019-s3-preconditions-for-release-publication-and-promotion.md) makes the store's error codes part of the contract rather than a deployment detail; it is exercised against a mock, and the bucket and retention rule it publishes into are provisioned by `infra/central`. Keep a concept in one owning document and link to it elsewhere. Do not copy full requirements between the goal, specification, ADRs, and runbook.
