...
`infra/bootstrap/` provisions the versioned remote-state bucket, and `infra/central/` decodes the deployment input and provisions the config bucket, source-state and delivery tables, FIFO queue and delivery DLQ, Slack credential containers, operational topic, runtime-failure queue, the chapter 05 IAM roles, and the log groups, dashboard, and alarms. It defines the Slack worker and recovery reconciler Lambdas when each function's exact package digest and S3 version are supplied. The worker consumes FIFO batches; the reconciler runs every five minutes with bounded observations and repairs. Watcher and regular dispatcher handlers remain milestones. `src/` holds both runtime composition roots, the recovery core, the worker core, and their adapters. [ADR-020](../adr/020-exact-application-version-gate-for-delivery.md) is implemented by a deterministic package builder, an append-only digest-key publisher, an exact S3 object-version input, and one `sha256:` worker application value. [ADR-007](../adr/007-central-slack-delivery-queue-and-worker.md) owns FIFO ordering, the worker capacity boundary, and the recovery state matrix. Packages accumulate until a retirement tool can prove both retention floors. Release publication has an S3 adapter already, because [ADR-019](../adr/019-s3-preconditions-for-release-publication-and-promotion.md) makes the store's error codes part of the contract rather than a deployment detail. Keep a concept in one owning document and link to it elsewhere. Do not copy full requirements between the goal, specification, ADRs, and runbook.
