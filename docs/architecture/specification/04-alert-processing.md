...
An intentional change to a configured feed URL uses a new feed name. State for
the old name remains durable until a separately reviewed retirement operation;
changing a URL in place must not silently carry validators or acquisition
history to a different source location.
...
suppresses only an identical deterministic identity; it does not collapse two
different revisions merely because their canonical announcement URL matches.
