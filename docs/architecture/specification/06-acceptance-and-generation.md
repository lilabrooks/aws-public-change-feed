...
- Given application-package publication, malformed or unsafe ZIP members,
  source drift from tracked `HEAD`, a missing or changed ADR-029 manifest, a
  missing configured module, and an unsupported handler declaration are each
  rejected before S3 write. The published exact version carries its byte
