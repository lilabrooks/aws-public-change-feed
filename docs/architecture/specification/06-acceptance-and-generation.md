...
- Given a direct shadow invocation, the expected release ID, application digest, and complete sorted feed-name set must match before feed work. The completed outcome set names that same feed set exactly. The runtime loads and fetches through the deployed watcher path but constructs only fresh in-memory feed, snapshot, announcement, candidate, and delivery stores. Its execution role has no DynamoDB, S3 write, queue, or secret action; a separate operator role can invoke only this function. The result exposes the invocation ID, bounded counts, route IDs, feed outcomes, and only a digest of sorted candidate IDs. Fixed refusal codes survive the Lambda boundary.
- Given application-package publication, a ZIP missing any of the five
  configured handler modules is rejected before S3 write. The published exact
  version carries both its byte digest and the null-framed configured-handler
  digest as metadata. A central Terraform plan rejects any shared or reconciler
  artifact selection whose exact-version read-back lacks either match.
