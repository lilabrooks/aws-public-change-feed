...
**Dispatch, SQS transport, Slack delivery, and reconciliation.** The dispatcher is implemented behind ports. It merge-orders and caps due work across scheduled states, validates exact stored requests and their byte limit, conditionally claims a state version and monotonically increasing generation, reuses active claims after uncertain sends, sends through the SQS FIFO adapter, and conditionally records queue acceptance. In-memory race tests cover lost claims, post-send write failure, and the full ABA cycle; moto covers the DynamoDB conditions and SQS message body, group, and dedupe attributes. The Lambda handler, metrics adapter, Slack worker, delivery states after `queued`, recovery reconciler, and rendered sample remain unbuilt.
...
What remains is the deployed runtime: the four Lambda handlers, their schedules and event source mappings, DynamoDB source and destination operations, S3 snapshot storage, Slack credential access, the worker state machine, and recovery. The delivery-table and SQS producer adapters now exist, though no function invokes them in AWS. Because no runtime is deployed yet, none of the IAM defects above had produced a failure; they would have surfaced at the dispatcher's first invocation.

**Production preflight.** Not started, and blocked on the milestones above.

No box is one edit away. The corpus one looks closest and is not: its remaining gap is labelled depth in the thin pairs rather than a threshold setting. The largest missing piece is runtime delivery after queue dispatch: the Slack worker, destination pacing, retries, terminal and unknown outcomes, recovery, Lambda packaging, and the schedules and event source mappings that wire the tested ports to the deployed resources.
