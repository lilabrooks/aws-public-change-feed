...
What remains is the rest of the deployed runtime: watcher and regular dispatcher handlers and schedules, DynamoDB source-state operations, S3 snapshot storage, package retirement, manual replay and DLQ recovery tools, and operator-run worker and reconciler deployments. The worker Lambda, reconciler Lambda and schedule, FIFO event-source mapping, package build and publication path, exact per-function artifact inputs, and rollout procedures now exist in source but have not been applied. Live AWS and Slack behavior therefore remains unproven.

**Production preflight.** Not started, and blocked on the milestones above.

No box is one edit away. The corpus one looks closest and is not: its remaining gap is labelled depth in the thin pairs rather than a threshold setting. The worker and reconciler slices now have source-defined handlers, metrics, packaging, and conditional Terraform resources. Their next evidence is deployment and controlled recovery and Slack preflight. The broader milestone still needs regular dispatcher scheduling, the remaining runtime adapters, manual replay, and DLQ recovery before it can close.
