...
No box is one edit away. The corpus one looks closest and is not: its remaining gap is labelled depth in the thin pairs rather than a threshold setting. The largest missing piece is the Terraform roots, because they are what turn the ports into deployed behavior, carry the retention rule and the bucket the concurrency suite needs, and unblock the evidence the last milestone requires.
