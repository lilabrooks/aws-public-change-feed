...
What remains is the rest of the deployed runtime: watcher, dispatcher, and reconciler handlers and schedules, DynamoDB source-state operations, S3 snapshot storage, package retirement, recovery tools, and an operator-run worker deployment. The worker Lambda, FIFO event-source mapping, package build and publication path, exact digest injection, and rollout procedure now exist in source but have not been applied. Live AWS and Slack behavior therefore remains unproven.

**Production preflight.** Not started, and blocked on the milestones above.

No box is one edit away. The corpus one looks closest and is not: its remaining gap is labelled depth in the thin pairs rather than a threshold setting. The worker slice now has the handler, metrics, packaging, append-only publication, and event-source definition that were its largest local gaps. Its next evidence is deployment and a controlled Slack preflight. The broader milestone still needs dispatcher scheduling, recovery, the remaining runtime adapters, and manual replay before it can close.
