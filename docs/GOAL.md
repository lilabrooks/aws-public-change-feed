...
Slice 2 now has an accepted capacity boundary. The configured 300-second Lambda timeout is the hard invocation bound, queue visibility must be at least six times that value, and the FIFO handler checks remaining invocation time before each record. `slack_request_timeout_seconds` bounds each blocking socket operation separately. A Lambda timeout after the sending claim becomes `delivery_unknown` through lease recovery. ADR-007 records the conditions that reopen this choice.

Application-package replay also has an accepted floor: retain content-addressed packages for at least 400 days and keep at least the newest 10. Rollout pauses candidate creation, drains actionable old-version work, deploys producer and worker roots with the same digest, and then resumes. Older unresolved evidence stays durable when its package is unavailable and requires an operator decision.
