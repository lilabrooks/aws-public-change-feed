...
What remains is the runtime half: the four Lambda functions, their schedules and event source mappings, and the DynamoDB/SQS/Secrets adapters that turn the in-memory ports into deployed behavior. Because no runtime is deployed yet, none of the IAM defects above had produced a failure; they would have surfaced at the dispatcher's first invocation.
