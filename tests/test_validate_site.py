...
class SiteValidatorTests(unittest.TestCase):
    ...
    def test_milestone_status_matches_the_completed_work(self):
        ...
        self.assertIn("milestone/6) | Closed |", readme)
        self.assertIn("the offline M5 maintenance milestone is closed", page)
        self.assertIn("M2 · Closed", page)
        self.assertIn("M3 · Passed", page)
        self.assertIn("M5 · Closed", page)
        self.assertIn("76 announcement rows and 88 response-page rows", goal)
        self.assertIn("**Offline maintenance readiness.**", goal)
        self.assertIn("product scope nor its operating mode", goal)
