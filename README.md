...
The Terraform roots are built and applied to the dev deployment. The config bucket, DynamoDB tables, FIFO queue, DLQ, IAM roles, log groups, alarms, and dashboard are provisioned. The delivery-table and SQS dispatch adapters exist behind tested ports. The Slack worker state machine is built behind Slack transport and credential ports, with exact release and application-artifact gates, bounded rendering, conditional outcomes, retries, and pacing tests. Those two ports now have production adapters: a Slack HTTP transport for both incoming-webhook and bot-token modes that pins the validated address while verifying the certificate against the approved hostname, and credential readers for both accepted secret stores, Secrets Manager and SSM SecureString. A received Slack status decides the delivery outcome, and a transient credential-store failure reschedules the record instead of resolving it. Both adapters are exercised against injected transports and clients, plus moto for the AWS request and response shapes, rather than live Slack. DynamoDB source-state operations, S3 snapshot storage, FIFO batch handling, the bounded metrics adapter, content-addressed Lambda packaging, artifact retention and rollout, the four Lambda handlers and schedules, and recovery remain implementation milestones. Accepted deployment rules now use the 300-second Lambda timeout as the worker capacity bound, require at least 1,800 seconds of queue visibility, and retain package artifacts for at least 400 days and 10 versions. No function invokes the adapters in AWS yet, so per-destination serialization still depends on the unbuilt FIFO event source mapping. Deployment, live feed acquisition, Slack delivery, recovery, load, and production preflight still require executable evidence.
