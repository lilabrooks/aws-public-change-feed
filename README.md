...
- [L-42](https://github.com/lilabrooks/aws-public-change-feed/issues/147) now has a source-defined, direct-invocation shadow evaluator with no durable-state authority, a scoped invoker, fixed refusal codes, and an exact preview/apply command for configuration rollback and forward restoration. Accepted ADR-026 requires all durable runtimes to stop before rollback and records the five-function application boundary. The first live invocation on 2026-09-05 failed before handler entry because the selected retained package predated `shadow_runtime.py`; source-state, delivery, and raw-snapshot read-backs were unchanged, and the fixed attempt was not repeated. Publication and Terraform now reject packages without the complete configured-handler contract. On 2026-09-06 an exact reviewed plan stopped all four triggers and watcher execution, the full 300-second quiescence window elapsed, and a second exact plan deployed one verified handler-complete package to all five Lambdas with both artifact guards. Three separately authorized identity inversions then reached the remediated handler and returned the exact release, application, and feed-set refusal codes. The following single valid sample fetched all four feeds, normalized 240 items, matched 8 route-scoped candidates, and returned `passed`; source-state, delivery, and raw-snapshot read-backs remained unchanged throughout. Configuration rollback to retained release `0ffc94ed…` then passed its compatibility probe, and its one shadow sample returned the same item count, candidate count, and candidate-identity digest. A preview-only retained-source replay also resolved the pre-exercise snapshot and exact historical release references without creating work. The pointer currently names the retained release, all runtimes remain stopped, forward restoration is previewed but unrun, and application rollback remains unrun.
