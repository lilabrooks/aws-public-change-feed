...
The implemented runtime now covers feed acquisition, normalization, announcement identity, deterministic matching, profile and route mapping, candidate construction, the durable outbox creation boundary, the DynamoDB delivery store, due-work dispatch, the SQS FIFO sender, the Slack worker, and bounded delivery recovery. Its corpus mixes real announcements taken from the four configured feeds with authored items covering shapes the recent feed window did not contain. No end-of-support announcement appeared in that window, so recall for that risk type rests on authored items alone.

The applied Terraform foundation includes the config bucket, DynamoDB tables, FIFO queue, DLQ, IAM roles, log groups, alarms, and dashboard. The delivery-table and SQS dispatch adapters exist behind tested ports. The Slack worker has a FIFO Lambda entrypoint and AWS composition root around its tested state machine, HTTP transport, and credential readers. Its handler binds the queue body, message ID, and message group to durable state, preserves the FIFO suffix after a failure, and stops before its invocation reserve is spent. The recovery reconciler now has a separate scheduled Lambda composition root. It observes four indexed states through bounded queries, sends only due scheduled work through the existing dispatcher, conditionally converts exact expired leases to `delivery_unknown`, reports old queued work without resending it, and fails visibly when a run reaches its observation or repair limit. Terraform gives the reconciler a five-minute schedule, 60-second timeout, reserved concurrency one, separate exact artifact inputs, two retries within 300 seconds, and a source-scoped standard runtime-failure queue. Both Lambda resources are conditional and remain unapplied until an operator publishes and supplies each exact package digest and S3 version. The deterministic Linux builder and append-only S3 publisher bind those deployments to reviewed bytes. Published packages accumulate because no retirement mechanism yet proves both retention floors. The watcher and regular dispatcher Lambda handlers, DynamoDB source-state operations, S3 snapshot storage, manual replay and DLQ recovery tools, live Slack evidence, load testing, and production preflight remain.
