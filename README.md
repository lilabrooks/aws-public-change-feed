...
- State and package cleanup removed the unused `dynamodb:TransactWriteItems` IAM action. Remaining work will make each source-state load name whether it wants a feed or announcement record, test that `__pycache__` directories and `.pyc` files stay out of Lambda packages, and document the order for changing the deployment host allowlist and configuration feed-host set.
